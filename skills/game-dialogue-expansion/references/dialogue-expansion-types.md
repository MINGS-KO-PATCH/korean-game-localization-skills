...
| 9. Tagged nested 4-byte length chunks (provisional) | Repeating 4-byte tag, `u32le` payload length, and payload; outer wrappers carry aggregate sizes | Sequential chunk-local payload lengths plus enclosing-file extents | Re-encode payload, rewrite its chunk length, then every enclosing aggregate size | Length must come from final bytes; structural walking must replace marker searching; unknown characters must fail |
